...
| 3 | **PSP refund not implemented** | CERTAIN | HIGH (no customer refund possible with real keys) | **HIGH** | Implement MoMo + VNPay refund API in `lib/payment/refund.ts` before issue 094 go-live. | CTO |
...
| 5 | **[VN] PDPD 2023 non-compliance (cross-border transfer to SG without mechanism; no DPIA; no breach runbook)** | MEDIUM | HIGH (fines, data deletion orders) | **HIGH** | Complete DPIA; add data residency disclosure in Privacy Policy; create 72h breach notification runbook; implement Booking PII anonymization sweeper (issue 090). | CEO + DPO |
...
| 12 | **External monitoring absent** | CERTAIN | MEDIUM (downtime discovered by users, not team) | **MEDIUM-HIGH** | Implement external uptime monitoring with PagerDuty-equivalent alerts before go-live. 2-minute detection target. | CTO |
