...
| 5 | **[VN] PDPD 2023 non-compliance (no DPIA; no breach runbook)** | MEDIUM | HIGH (fines, data deletion orders) | **HIGH** | **Cross-border transfer risk MITIGATED (2026-06-21)** — Vercel Pro + Neon + Upstash (Singapore) requires CDTIA filing under PDPL 2025 Art. 25. Filing accepted per ADR-020 D11 (~$2-5K one-time, 60-day window with MPS A05). FPT Cloud backup path eliminates CDTIA entirely. Remaining: complete DPIA; add data residency disclosure in Privacy Policy; create 72h breach notification runbook; implement Booking PII anonymization sweeper (issue 090). See ADR-020 D7, DS-017 §3.1. | CEO + DPO |
