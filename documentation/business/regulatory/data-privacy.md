...
| Provider | Notes |
|----------|-------|
| Viettel Cloud (IDC) | Largest VN provider, 70% market share (with top 5) |
| FPT Cloud | Competitive managed services |
| VNG Cloud | Multi-region (HCM + Hanoi, June 2024) |
| CMC Cloud | USD 1B deal with Samsung C&T (Dec 2024) |
...
| Migration plan to VN cloud | Within 6mo | Have plan ready, execute if MPS requests |
| Define retention schedules | Before launch | Booking data, payment data, OTP logs |

### CDTIA Filing Status (as of 2026-06-18)

**NOT FILED.** Deadline has likely passed per PDPL 2025 Art. 25 (60 days after commencing cross-border transfer). Cross-border processors requiring coverage:

1. **Vercel** (Singapore) — application hosting, serves all user requests
2. **Upstash Redis** (Singapore/US) — rate-limit counters keyed by IP address (PII under PDPL 2025)
3. **Resend** (US) — email delivery with customer email addresses
4. **Neon** (if applicable) — database hosting

**ACTION:** File CDTIA immediately. Consult legal counsel on penalty exposure for late filing. MPS can order immediate cessation of cross-border transfers (Art. 26).
