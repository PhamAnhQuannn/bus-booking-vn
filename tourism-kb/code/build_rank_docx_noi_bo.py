...
Render raw/<slug>/noi-bo/rank_noi_bo_nha_hang.json (Google + VQS) thành docx xếp hạng để
TỰ chọn lịch trình. KHÔNG gọi API. Ghi vào raw/<slug>/docx/ (gitignored) — KHÔNG output/.

Chạy:  PYTHONIOENCODING=utf-8 python tourism-kb/code/build_rank_docx_noi_bo.py tourism-kb/raw/<slug>/scrape
"""
...
SRC = os.path.join(CITY_DIR, "noi-bo", "rank_noi_bo_nha_hang.json")
OUT = os.path.join(CITY_DIR, "docx", "Rank-Noi-Bo-Nha-Hang-%s.docx" % _CITY_FILE)
...
_t = doc.add_paragraph().add_run("⚠ NỘI BỘ — XẾP HẠNG NHÀ HÀNG %s (Google, %s)" % (CITY.upper(), d.get("lay_luc", "")))
_t.bold = True
_t.font.size = Pt(14)
...
COLS = ["STT", "Nhà hàng", "★", "Lượt", "VQS", "Địa chỉ", "Google"]
