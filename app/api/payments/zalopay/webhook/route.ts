/**
 * POST /api/payments/zalopay/webhook — ZaloPay IPN receiver.
 *
 * Thin gateway-specific shell, parallel to the MoMo receiver. Resolves the
 * gateway via getGatewayFor('zalopay', baseUrl) — local fake-gateway stub while
...
async function handler(req: NextRequest): Promise<Response> {
  ...
  return processPaymentWebhook({
    rawBody,
    gateway: getGatewayFor('zalopay', baseUrl),
    adapter: 'zalopay',
    proto,
    host,
  });
