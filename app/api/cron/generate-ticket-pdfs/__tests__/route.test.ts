...
function makeRequest(headers?: Record<string, string>): NextRequest {
  return new NextRequest('http://localhost/api/cron/generate-ticket-pdfs', {
    method: 'GET',
    headers,
  });
}
...
describe('GET /api/cron/generate-ticket-pdfs', () => {
  describe('auth', () => {
  ...
  describe('advisory-lock + JobRunLog wrapper', () => {
    it('runs through runJob with the ticket-pdf lock key', async () => {
    ...
    it('surfaces skipped_locked from a concurrent tick that finds the lock held', async () => {
      ...
      const res = await GET(makeRequest());
      expect(res.status).toBe(200);
      expect(await res.json()).toEqual({ rowsAffected: 0, status: 'skipped_locked' });
    });
