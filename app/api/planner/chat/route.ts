 ...
 *   error   {message, fallbackHref}       — 429/quota/no_key/env -> fallback thủ công (Mục C)
 *   done
 *
...
// Đường thoát hiểm không-AI khi chat lỗi: client đọc fallbackHref trong error frame để bật
// nút "Tự chọn lịch trình" (luồng chip tất định → /api/planner/itinerary, không cần Gemini/getEnv).
const FALLBACK_HREF = '/tro-ly-du-lich?manual=1';
...
// Một khung SSE `error` (HTTP 200) cho lỗi PRE-STREAM (vd getEnv throw): biến 500 trần thành
// bong bóng lỗi có kiểm soát + kèm fallbackHref, để client hiện thông báo + nút thủ công thay vì
// nuốt request. Client parse `event: error` giống hệt nhánh lỗi trong stream.
function preStreamError(message: string): Response {
  return new Response(sse('error', { message, fallbackHref: FALLBACK_HREF }), {
    status: 200,
    headers: SSE_HEADERS,
  });
}
...
export async function POST(req: NextRequest): Promise<Response> {
  try {
    ...
    // Lỗi PRE-STREAM (trước khi mở stream) — điển hình getEnv() throw khi thiếu env prod-required
    // trên `next start`/prod cấu hình lệch. Trước đây ném ra 500 TRẦN (ngoài mọi try/catch) → client
    // chỉ thấy res không OK → "đang bận" mà không có Sentry/fallback. Giờ: 1 khung SSE error graceful.
    captureException(err, { route: 'planner/chat', code: 'env_config' });
    return preStreamError('Trợ lý đang bận, bạn thử nhắn lại sau nhé.');
  }
}
