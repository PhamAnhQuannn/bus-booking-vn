...
Distilled from the codebase Mistake Log. Full per-lesson post-mortems live in `~/.claude/projects/D--Bus-Booking/memory/lessons/app/<domain>/` (indexed by `memory/lessons/app/_index.md`); see `## Memory & Lessons Map` below.
...
Detailed history is no longer inlined here. It lives in the persistent memory store
(`~/.claude/projects/D--Bus-Booking/memory/`), indexed by `MEMORY.md` (loaded every session)
and retrieved on demand — read a leaf only when a task matches its hook. That dense-index +
read-on-demand IS the retrieval mechanism (no engine).

- `memory/MEMORY.md` — master retrieval map (facts + folder addresses + open/active rollups).
- `memory/CONVENTIONS.md` — write-protocol: where plans / issues / history / lessons / facts go.
- `memory/facts/<group>/` — durable one-fact memories (launch, workflow, dev-env, tourism, compliance, infra); index `facts/_index.md`.
- `memory/lessons/app/<domain>/` — the 71 app post-mortems, one file each; index `lessons/app/_index.md`. Their distilled digest is `## Project Rules` above (always loaded).
- `memory/plans/<slug>/` — plans to execute; one subfolder per plan.
- `memory/issues/<slug>/` — bugs found running the live product; one subfolder each (distinct from the repo's `issues/` PRD tasks).
- `memory/history/<YYYY-MM-DD>/<task>.md` — completed work, one file per task per day.

**Tourism KB** keeps its OWN brain under `tourism-kb/memory/` (plans/issues/history/lessons),
indexed by `tourism-kb/CLAUDE.md` — not this store.

New app lesson → write `memory/lessons/app/<domain>/<date>-<slug>.md`, add its line to that
folder's `_index.md`, and if it generalizes, one line to `## Project Rules` here.
