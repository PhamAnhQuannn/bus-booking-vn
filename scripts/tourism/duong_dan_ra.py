...
Do bang so, 2026-08-01 — chin duong dan hop ly ma mot builder co the bi tro vao,
tam duong dan trong so do KHONG bi ignore:

    docs/Huong-Dan-Da-Lat.md        da ignore   <- chi ten quy uoc duoc phu
    ...
    documentation/tourism-v2/x.md   COMMIT DUOC
...
NOI — `scripts/tourism/diem_den_chot.json` roi vao thu muc script chu khong phai
thu muc du lieu, va `scratch_out*.txt` khong khop mau nao. Nen thay vi doan tiep
xem file se roi o dau, chan ngay tai luc GHI.
...
======================================
`.tourism-data/` giu 14.328 so di dong Viet Nam that, `documentation/tourism/`
giu 416. Voi ho kinh doanh mot nguoi — vuon dau, tiem thue xe may — "so doanh
...
# Thu muc goc repo, suy ra tu vi tri file nay (scripts/tourism/ -> len hai cap).
# Khong goi `git rev-parse` — module nay phai chay duoc ca khi khong co git.
GOC = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

# Hai thu muc duoc ghi thoai mai o moi do sau. Ca hai deu da nam trong
# .gitignore; danh sach nay va .gitignore phai noi cung mot dieu.
THU_MUC_CHO_PHEP = ("documentation/tourism", ".tourism-data")

# `docs/` chi mo cho DUNG bon tien to nay — ba ban giao .docx dang phat hanh va
# ban .md tuong ung. Chung khop luat ignore theo TEN trong .gitignore
# (`/docs/Huong-Dan-*` ...), nen ghi vao day van an toan. Bat ky ten nao khac
# trong `docs/` deu bi tu choi, vi `docs/` con chua hon 240 tai lieu that cua du
# an (docs/qa/ mot mot bao cao review, docs/current-status/, docs/working-track/).
...
def duoc_phep(duong_dan):
    ...
    if rel.startswith("docs/"):
        ...
        return any(ten.startswith(t) for t in TIEN_TO_DOCS)
    return False
...
def kiem_loi_ra(duong_dan):
    ...
    sys.stderr.write(
        ...
        "    .tourism-data/...              (moi du lieu tho va trung gian)\n"
        "    documentation/tourism/...      (tai lieu ban giao)\n"
        "    docs/{{Huong-Dan|Diem-Den|Nha-Hang|Khach-San}}-*   (ban phat hanh)\n"
        "\n"
        "  Neu that su can mot noi moi: them luat vao .gitignore VA vao\n"
        "  THU_MUC_CHO_PHEP trong scripts/tourism/duong_dan_ra.py cung luc.\n"
