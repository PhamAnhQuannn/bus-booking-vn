...
// Credentials from env only — no literal password default (secret-hygiene: repo goes public during
// /ship). Username default is the non-secret seed code. Set SMOKE_OP_PASSWORD when running locally.
const OP_USERNAME = process.env.SMOKE_OP_USERNAME ?? 'PB-0001';
const OP_PASSWORD = process.env.SMOKE_OP_PASSWORD;
...
export async function operatorCrawl(baseUrl: string): Promise<Check[]> {
  const out: Check[] = [];
  if (!OP_PASSWORD) {
    return [{ name: 'operator crawl (skipped)', ok: false, optional: true, detail: 'set SMOKE_OP_PASSWORD to enable' }];
  }
