 ...
 * bookingRef from the returned payUrl's orderId, then manually posts a
 * properly-signed IPN body (sandbox creds are public) to simulate delivery.
 */
...
test.describe('MoMo booking — webhook IPN processing', () => {
  test('valid paid IPN transitions booking to paid (200 response)', async ({
    ...
    // Seed an online (momo) booking to get a bookingRef we can use.
    // Online-only (Issue 039): the cash rail was removed; the momo rail creates
    // an `awaiting_payment` booking and returns a payUrl carrying the bookingRef
    // as its orderId query param.
    const holdInfo = await seedHoldViaUI(request, baseURL);
    test.skip(!holdInfo || !holdInfo.holdId, 'No trips available — re-run prisma db seed');
    if (!holdInfo) return;
    ...
    // 502 = MoMo gateway unreachable in this env — nothing to webhook against.
    test.skip(!initiateRes.ok(), 'Booking initiate failed (gateway unreachable?)');
    ...
    test.skip(!initiateData.payUrl, 'No payUrl in response');
    if (!initiateData.payUrl) return;

    // bookingRef is the orderId query param on the gateway payUrl.
    const orderId = new URL(initiateData.payUrl).searchParams.get('orderId') ?? '';
    const refMatch = orderId.match(/BB-\d{4}-[0-9a-z]{4}-[0-9a-z]{4}/);
    test.skip(!refMatch, 'Could not extract bookingRef from payUrl orderId');
    if (!refMatch) return;
