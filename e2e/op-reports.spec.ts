...
const OP_A_PHONE = '+8490xxxxxx3';
const OP_A_PASSWORD = 'BBOp2026!';
const OP_B_PHONE = '+8490xxxxxx4'; // used only for IDOR test — operator user, not customer
...
async function prepareReports(): Promise<PrepareCtx> {
  ...
  try {
    ...
    // ---- Operator A ----
    // Upsert Operator A
    const opARow = await client.query<{ id: string }>(
    ...
    let opAId: string;
    if (opARow.rows.length > 0) {
      opAId = opARow.rows[0].id;
    } else {
    ...
    await client.query(
      `INSERT INTO "OperatorUser"
       ...
       VALUES (gen_random_uuid()::text, $1, $2, $3, $4, $5, 'admin', false, 'Reports Op A Admin')
    ...
    const opBRow = await client.query<{ id: string }>(
      `INSERT INTO "Operator" ("id","legalName","contactPhone","contactEmail")
    ...
    await client.query(
      `INSERT INTO "OperatorUser"
         ("id","phone","contactPhone","notificationPhone","passwordHash","operatorId","role","requiresPasswordChange","displayName")
       VALUES (gen_random_uuid()::text, $1, $2, $3, $4, $5, 'admin', false, 'Reports Op B Admin')
       ON CONFLICT (phone) DO UPDATE
         SET "passwordHash" = $4, "requiresPasswordChange" = false`,
    ...
    const routeRow = await client.query<{ id: string }>(
      `INSERT INTO "Route" ("id","operatorId","origin","destination","durationMinutes")
       VALUES (gen_random_uuid()::text, $1, 'Hà Nội', 'Hải Phòng', 120)
       ON CONFLICT DO NOTHING RETURNING id`,
      [opAId]
    );
    let routeId: string;
...
test.describe('Operator revenue + payout reports (Issue 016)', () => {
  ...
  test('Case 1: revenue report page renders trip row with correct amounts', async ({ page }) => {
    ...
    await page.request.post('/api/op/auth/login', {
      data: { phone: OP_A_PHONE, password: OP_A_PASSWORD },
  ...
  test('Case 5: cross-operator IDOR — Op B gets 404 on Op A payout retry', async ({ request }) => {
    ...
    await request.post('/api/op/auth/login', {
      data: { phone: OP_B_PHONE, password: OP_B_PASSWORD },
      headers: { 'X-CSRF-Token': csrf },
    });
