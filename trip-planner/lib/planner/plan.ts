...
function scoreDestination(rec: KbRecord, req: TripRequest): number {
  ...
  if (req.interests?.length) {
    // khớp interests với category VẬT LÝ + nhãn TRẢI NGHIỆM (trai_nghiem) — nhãn là tín hiệu mạnh hơn
    const hay = `${rec.category?.primary ?? ""} ${(rec.category?.secondary ?? []).join(" ")} ${d.trai_nghiem ?? ""}`.toLowerCase();
    if (req.interests.some((i) => hay.includes(i.toLowerCase()))) s += 2;
  }
  return s;
}
...
function growCompact(regs: Reg[]): { kept: Reg[]; dropped: Reg[] } {
...
function buildDayChunks(store: Store, req: TripRequest, days: number, perDay: number): { chunks: KbRecord[][]; notes: string[] } {
  ...
  const regs: Reg[] = [...groups.entries()].map(([key, pts0]) => {
    // pts sort điểm giảm dần -> quality quyết TRONG cụm (A1) khi packDays cap budget
    const pts = [...pts0].sort((a, b) => (scoreOf.get(b.id)! - scoreOf.get(a.id)!) || (a.id < b.id ? -1 : 1));
  ...
  const { kept, dropped } = growCompact(regs);
  ...
  // C1 marquee TRONG cụm compact: cụm nhỏ + lệch core -> ngày RIÊNG. KHÔNG bảo vệ cụm xa (đã bị growCompact loại).
  const med = median(kept.map((r) => r.distTam));
  const marquee = kept.filter((r) => r.card <= MARQUEE_CARD_MAX && med > 0 && r.distTam > FAR_FACTOR * med);
  let protReg: Reg[] = [];
  if (marquee.length && days >= 3) protReg = marquee.slice(0, Math.max(0, days - 1)); // để lại >=1 ngày cho phần còn lại

  const rest = kept.filter((r) => !protReg.includes(r));
  const restDays = Math.max(1, days - protReg.length);
  const restChunks = packDays(store, macroOrder(rest, tam), restDays, perDay);
...
function recommendRestaurants(store: Store, centroid: LL, capKm: number, n: number): KbRecord[] {
...
export function buildItinerary(req: TripRequest, store?: Store): Itinerary {
  ...
  // GỢI Ý quán ăn: top-N theo ảnh hưởng trong vùng chuyến đi.
  const restaurants: PlaceRef[] = recommendRestaurants(st, centroid, dynamicCapKm(tripSpanKm), clampInt(req.days * 3, 6, 12)).map(toPlaceRef);

  const hotel: PlaceRef | null = hotelRec
    ? {
        ...toPlaceRef(hotelRec),
        note: [hotelRec.ext?.hotel?.phan_khuc, hotelRec.ext?.hotel?.so_phong ? `${hotelRec.ext?.hotel?.so_phong} phòng` : null]
