...
type Group = { label: string; items: Stamped[] };

/** Nhóm hội thoại theo ngày VN: Hôm nay / Hôm qua / 7 ngày trước / Cũ hơn. */
function groupByDate(list: ConversationMeta[], nowMs: number): Group[] {
  ...
  return [
    ...
    { label: 'Hôm qua', items: buckets.yest },
    { label: '7 ngày trước', items: buckets.week },
    { label: 'Cũ hơn', items: buckets.older },
  ].filter((g) => g.items.length);
}

const CAPABILITIES = [
  { icon: '📍', text: 'Gợi ý điểm đến phù hợp với bạn' },
  { icon: '🗓️', text: 'Lên lịch trình chi tiết theo ngày' },
  { icon: '🚌', text: 'Đặt vé xe, gợi ý nhà xe uy tín' },
  { icon: '🍜', text: 'Gợi ý ăn uống, lưu trú, vui chơi' },
  { icon: '⏱️', text: 'Ước tính chi phí và thời gian di chuyển' },
];
  ...
  if (collapsed) {
    return (
      <div className="hidden shrink-0 flex-col items-center gap-3 border-r border-[#E4D8C9] bg-[#FDFAF7] px-2 py-4 lg:flex">
        <button type="button" onClick={onToggleCollapse} aria-label="Mở lịch sử"
        ...
        <button type="button" onClick={onNew} aria-label="Cuộc trò chuyện mới"
...
function HistoryView(props: {
  ...
  function handleRename(id: string, current: string) {
    setMenuId(null);
    const next = typeof window !== 'undefined' ? window.prompt('Đổi tên cuộc trò chuyện:', current) : null;
    if (next && next.trim() && next.trim() !== current) onRename(id, next.trim());
  }
  function handleDelete(id: string) {
    setMenuId(null);
    if (typeof window !== 'undefined' && window.confirm('Xoá cuộc trò chuyện này?')) onDelete(id);
  }
  function handleClearAll() {
    if (typeof window !== 'undefined' && window.confirm('Xoá toàn bộ lịch sử trò chuyện?')) onClearAll();
  }
    ...
    <div className="flex min-h-full flex-col p-3">
      <div className="flex items-center justify-between px-1 pb-2">
        <span className="flex items-center gap-1.5 text-sm font-semibold">
          <span aria-hidden>🕐</span> Lịch sử trò chuyện
        </span>
        <button type="button" onClick={onToggleCollapse} aria-label="Thu gọn"
      ...
      <div className="mt-3 flex flex-col gap-4">
        {groups.map((g) => (
          <div key={g.label}>
            <p className="px-1 pb-1.5 text-xs font-semibold uppercase tracking-wide text-muted-foreground">{g.label}</p>
                ...
                <li key={c.id} className="relative">
                  <button type="button" onClick={() => onOpen(c.id)}
                    ...
                    <span onClick={(e) => { e.stopPropagation(); setMenuId(menuId === c.id ? null : c.id); }}
                      role="button" tabIndex={0} aria-label="Tùy chọn"
                  ...
                  {menuId === c.id ? (
                    <div className="absolute right-2 top-9 z-popover w-32 overflow-hidden rounded-lg border border-border bg-popover shadow-e3">
                      <button type="button" onClick={() => handleRename(c.id, c.title)}
                        className="block w-full px-3 py-2 text-left text-sm hover:bg-primary/5">Đổi tên</button>
                      <button type="button" onClick={() => handleDelete(c.id)}
                        className="block w-full px-3 py-2 text-left text-sm text-destructive hover:bg-destructive/5">Xoá</button>
                    </div>
                  ) : null}
                </li>
              ))}
      ...
      {authStatus === 'guest' ? (
        <div className="mt-4 rounded-2xl border border-primary/25 bg-primary/[0.06] p-4">
          <p className="flex items-center gap-1.5 text-sm font-semibold"><span aria-hidden>⭐</span> Nâng cấp trải nghiệm</p>
          <p className="mt-1.5 text-xs leading-relaxed text-muted-foreground">
            Đăng nhập để lưu lịch sử, đồng bộ trên mọi thiết bị và nhận gợi ý cá nhân hoá.
          </p>
  ...
  return (
    <div className="flex flex-col gap-5 p-5">
      <p className="flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-widest text-primary">
        <span aria-hidden>🧭</span> AI Trợ lý du lịch
      </p>
      <h2 className="text-2xl font-bold leading-tight">
        Xin chào!<br />Mình là BBVN AI <span aria-hidden>👋</span>
      </h2>
      ...
      <div className="flex items-start gap-3 rounded-2xl border border-[#F0E9E1] bg-white p-3.5 shadow-e1">
        ...
        <p className="text-[13px] leading-relaxed text-muted-foreground">
          Mình sẽ giúp bạn lên kế hoạch chuyến đi hoàn hảo từ A–Z chỉ trong vài phút.
        </p>
      </div>

      <div>
        <p className="text-sm font-semibold">Mình có thể giúp bạn:</p>
        <ul className="mt-3 flex flex-col gap-2.5">
          {CAPABILITIES.map((c) => (
            <li key={c.text} className="flex items-center gap-2.5 text-[13px]">
              <span className="grid h-6 w-6 shrink-0 place-items-center rounded-md bg-primary/10 text-xs" aria-hidden>{c.icon}</span>
      ...
      <p className="text-[11px] leading-relaxed text-muted-foreground">
        BBVN AI sử dụng công nghệ trí tuệ nhân tạo để mang đến trải nghiệm tốt nhất cho bạn.
      </p>
    </div>
  );
}
