...
function GioChip({ r }: { r: DtoRestaurant }) {
  if (!r.goi_truoc && r.gio_mo) {
    return <span className="rounded-full bg-success px-2 py-0.5 text-[13px] font-bold text-success-foreground">✓ Mở {isAllDay(r.gio_mo) ? 'cả ngày' : r.gio_mo}</span>;
  }
  return <span className="rounded-full bg-muted px-2 py-0.5 text-[13px] font-bold" style={{ color: SOFT }}>Giờ: gọi trước để hỏi</span>;
}
  ...
  return (
    <div className="max-w-[720px] rounded-[10px] border border-border bg-cream px-3.5 py-3 text-[13px] @[1100px]:max-w-none">
      <div className="flex items-baseline justify-between gap-2">
        ...
        <span className="shrink-0 text-[13px]" style={{ color: FAINT }} title={`${r.nguon} nguồn dữ liệu`}>{r.nguon} nguồn</span>
      </div>
...
export function RestaurantsList({
  ...
  return (
    <div className="@container flex flex-col gap-3 p-3">
      <div className="text-[13px] font-bold uppercase tracking-wide" style={{ color: SOFT }}>
