...
export function ItineraryCard({ dto, activeDay, selected, onHoverItem, onToggleDay }: Props) {
  ...
  return (
    <div className="@container w-full overflow-hidden rounded-2xl border border-border bg-background">
            ...
            <div key={d.day}>
              ...
              {open ? (
                <div className="v5-daybody mt-2 px-3">
                  {d.items.map((it) => (
                    <Row key={it.order} it={it} day={d.day} active={selected?.day === d.day && selected?.order === it.order} onHoverItem={onHoverItem} />
                  ))}
                </div>
              ) : null}
            </div>
