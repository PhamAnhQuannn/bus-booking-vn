...
function computeMapH(H_i: number, mapW: number, variant: Props['variant']): number {
  ...
  const plan = H_i - 30 - 24 - mapH; // tabs 30 + 2 gap 12
  if (plan < 280) mapH = Math.max(200, H_i - 30 - 24 - 280);
  return mapH;
}

export default function PlannerPane({ dto, activeDay, hoveredOrder, selected, onPinClick, onCloseSheet, onSelectDay, onHoverItem, variant, onOpenFull, pulseKey }: Props) {
  ...
  useEffect(() => {
    ...
    const ro = new ResizeObserver(() => {
      ...
      setMapH(Math.round(computeMapH(H_i, mapW, variant)));
    });
    ro.observe(el);
  ...
  return (
    <div
      ref={rootRef}
      className="flex h-full min-h-0 flex-col gap-3 overflow-hidden rounded-[20px] border border-border bg-background p-3 shadow-[0_2px_12px_rgba(30,36,51,0.06)]"
    >
      ...
      {short ? (
        ...
        <div className="shrink-0 overflow-hidden rounded-xl border border-border" style={{ height: mapH }}>
          <PlannerMap
            dto={dto}
            activeDay={activeDay}
      ...
      <div className="min-h-0 flex-1 overflow-y-auto">
        <ItineraryCard dto={dto} activeDay={activeDay} selected={selected} onHoverItem={onHoverItem} onToggleDay={onSelectDay} />
      </div>
    </div>
  );
}
