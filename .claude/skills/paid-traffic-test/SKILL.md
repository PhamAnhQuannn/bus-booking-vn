---
name: paid-traffic-test
description: Run paid Meta/Google/Reddit ads to measure CPC + CTR + intent — proves cold-traffic demand. Outputs to `docs/inception/paid-traffic-<project>.md`. Reads `/project-classify` to skip XS/S. Use when user says "paid traffic test", "ad smoke test", "FB ad demand check", "cold traffic test", "/paid-traffic-test", or after `/landing-page-test` if organic weak. Distinct from `/smoke-test` (Playwright browser verification of a running app — unrelated despite the legacy "smoke" wording).
output_size:
  XS: skip
  S: skip
  M: 1h
...
# /paid-traffic-test — Paid Cold-Traffic Demand Test

Invoke as `/paid-traffic-test`. Cold traffic = real signal. Friends ≠ market.
...
## Output
Write `docs/inception/paid-traffic-<project>.md`:

```markdown
# Paid Cold-Traffic Test — <project>
